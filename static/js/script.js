...
$('#train-form').on('submit',function(e){
		...
		// 	$('#loss_plot').append("<center><img src="+data.loss_plot+ " alt='Loss History Chart' width='400'></center>");
		// 	$('#acc_plot').append("<center><img src="+data.acc_plot+ " alt='Accuracy History Chart' width='400'></center>");
		// 	$('#preproccessing').show();
		// 	$('#hasil_training').show();
...
$('#train-form2').on('submit',function(e){
	e.preventDefault();
	var file_train = new FormData($('#train-form2')[0]);
	$('.button-update-newest').hide();
	$('.load-icon-train2').show();
	$.ajax({
		// data : file_train, 
		contentType: false,
		cache: false,
	...
	.done(function(data){
		// location.reload(true);
		$('.load-icon-train2').hide();
		$('.hasil_prediksi2').show();
...
$('#test-form').on('submit',function(e){
		...
		.done(function(data){
			...
			var table = $(".display").append("<thead><tr><th>No</th><th>Title</th><th>Label Score</th><th>Prediction</th></tr></thead><tbody>");
			$.each(data.data_output, function(a, b){
				table.append("<tr><td>"+b.id+"</td><td>"+b.title+"</td><td>"+b.label_score+"</td><td>"+b.prediction+"</td></tr>")
			});
			$(".display").append("</tbody><tfoot><tr><th>No</th><th>Title</th><th>Label Score</th><th>Prediction</th></tr></tfoot>");
			$('#empTable').DataTable({
				scrollY: "450px",
				scrollX: true,
				scrollCollapse: true,
...
// $(function () {	
// 	$('#train_submit2').on('click', function () {
// 		$('#train_submit2').removeAttr("hidden");
// 		$("#train_submit2").attr("hidden", true);
// 	});
// });